...
Го не требует заключать {\em условие} оператора \code{if} в скобки, зато фигурные скобки вокруг {\em тела} ифа нужны всегда. Открывающую фигурную скобку «\code{\{}» можно воспринимать, поэтому, как слово \code{THEN}.

Нельзя заканчивать строку сразу после слова else~"--- лексический анализатор Го поставит <<;>> в конец такой строки, и ничего не сработает\footnote{Синтаксис первой версии языка Го подразумевал, что операторы отделяются друг от друга точкой с запятой. В дальнейших версиях они перестали быть обязательными~"--- компилятор Го в ходе лексического анализа ставит точку с запятой в конец каждой строки, если только она заканчивается на символ, теоретически предусматривающий после себя точку с запятой.}. Поэтому формат только такой, писать \code{else} с новой строки нельзя:
\begin{gocode}
if a < b {
  x = 5
} else if a > b {
...
Такая запись считается допустимой, но среды разработки, как правило, всё равно разворачивают её в пять строк.
...
Своеобразная особенность языка Го: сразу после слова \code{if} можно написать оператор присваивания или быстрое объявление (через «:=») и поставить точку с запятой. После этого как ни в чём ни бывало пишется условие.
...
Переменная будет доступна до самого конца оператора \code{if}, то есть и в ветвях \code{else if} и \code{else}. После \code{else if} также допустимо присваивание. Вообще объявленные таким образом переменные существуют от места их объявления до конца всего оператора \code{if}.
